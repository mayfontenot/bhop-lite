...
function ENT:Draw()
	...
	render.StartBeam(5) --bottom, this makes a single beam, less memory usage
		render.AddBeam(b1, 2, 0, color)
		render.AddBeam(b2, 2, 0, color)
		render.AddBeam(b3, 2, 0, color)
		render.AddBeam(b4, 2, 0, color)
		render.AddBeam(b1, 2, 0, color)
	render.EndBeam()

	render.StartBeam(5) --top
		render.AddBeam(t1, 2, 0, color)
		render.AddBeam(t2, 2, 0, color)
		render.AddBeam(t3, 2, 0, color)
		render.AddBeam(t4, 2, 0, color)
		render.AddBeam(t1, 2, 0, color)
	render.EndBeam()

	render.DrawBeam(b1, t1, 2, 0, 0, color) --y
	render.DrawBeam(b2, t2, 2, 0, 0, color)
	render.DrawBeam(b3, t3, 2, 0, 0, color)
	render.DrawBeam(b4, t4, 2, 0, 0, color)
end